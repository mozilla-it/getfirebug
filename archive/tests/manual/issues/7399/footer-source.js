(function(w,d,undefined){
  ...
  // I'm the footer JavaScript.
  // I don't contain lots of code. Just some nice lorem ipsum.
  ...
  function dontDoAnythingExceptWeirdThingsForSomeKindaMusement() {
    var iFirst = 1,
        iSecond = 3;

    if ( (new Date()).getSeconds() % iSecond ) {
      console && console.log('i second, iSecond is iSecond.');
      if ( (new Date()).getMinutes() === iFirst ) {
        console && console.log('i See.');
      }
    }  

    // Use jQuery, they said.
    // Everything works with jQuery, they said.
    if ( w.$ ) {
      return $(iFirst) === $(iSecond-2);
    }

  }

  w.setTimeout(dontDoAnythingExceptWeirdThingsForSomeKindaMusement, 1500);


  // Oh and just to make sure, that it's not just another
  // it sticks on the bottom bug, let's just add that same
  // random lorem ipsum my emmet just created for the top
  // and put it straight to the floor.
/*
  Consectetur porro ad voluptates porro quam earum! A non unde.
  Elit possimus ipsum sequi elit mollitia. Ipsum culpa reiciendis molestias?
  Dolor perferendis ab tenetur alias quibusdam. Nihil sed quod delectus?
