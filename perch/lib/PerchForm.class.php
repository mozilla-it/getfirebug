...
class PerchForm
{
	...
	public function text($id, $value='', $class='', $limit=false, $type='text')
	{
		$this->fields[] = $id;
		...
		$s	= '<input type="'.$type.'" id="'.$this->html($id, true).'" name="'.$this->html($id, true).'" value="'.$this->html($this->value($value), true).'" class="text '.$this->html($class, true).'"'.$limit.' />';
	...
	public function textarea($id, $value='', $class='')
	{
		$this->fields[] = $id;
		
		if ($this->display_only) return nl2br($this->html($value));
		
		$s	= '<textarea id="'.$this->html($id, true).'" name="'.$this->html($id, true).'"  class="text '.$this->html($class, true).'" rows="6" cols="40">'.$this->html($this->value($value)).'</textarea>';
